...
def convolve2d(img, kernel):
    ...
    flipped_image = utils.flip2d(img, axis = None)
    padded_image = utils.zero_pad(flipped_image, 1, 1)
    padX, padY = np.array(padded_image).shape
    kernel_x, kernel_y = np.array(kernel).shape
    img_conv = np.zeros(np.array(img).shape)
    
    for i in range(padX-kernel_x):
        for j in range(padY-kernel_y):
            #img_conv = 0
            for k in range(kernel_x):
                for l in range(kernel_y):
                    img_conv[i][j] += ((kernel[k][l] * padded_image[i-kernel_x+k][j-kernel_y+l])) 

            #img_conv[i][j] = final_img
...
def normalize(img):
    ...
    # TODO: implement this function.
    img1 = np.asarray(img)
    normalized_img = (img1 - np.amin(img1))/(np.amax(img1) - (np.amin(img1)))
   # normalized_img = (img - min(img))/(max(img) - min(img))
   # normalized_img = (img.flatten - min(img.flatten()))/(max(img.flatten()) - min(img.flatten()))
