...
def convolve2d(img, kernel):
    ...
    padded_image = utils.zero_pad(flipped_image, pwx = 1, pwy = 1)
    padX, padY = np.asarray(padded_image).shape
    kernel_x, kernel_y = np.asarray(kernel).shape
    img_conv = np.zeros(np.array(img).shape)
    
    for i in range(0,padX):
        #print(i)
        for j in range(0,padY):
            img_conv[i][j] = (np.dot(kernel, (utils.crop(padded_image,i,i+kernel_x,j,j+kernel_y)))).sum()
